...
{
    public class StockIndicatorsFeatureExtraction : IFeatureExtractor<StockData>
    {
        ...
        public StockIndicatorsFeatureExtraction(
            int timesampling = 4,
            int slowEma = 32,
            int fastEma = 16,
            int macdEma = 12,
            ...
            bool normalize = false)
        {
            ...
            Encoding = $"Indicators-MACD({_slowEma},{_fastEma},{_macdEma},{_macdSlope})VWAP({_vwap},{_vwapSlope})RSI({_rsi},{_rsiSlope})CMF({_cmf},{_cmfSlope}),n{_normalize}-v{_version}";
        }
        ...
        public double[] Extract(IEnumerable<(StockData Data, float Weight)> data)
        {
            ...
            if(_normalize)
            {
                var normalized = Normalize(macdVals)
